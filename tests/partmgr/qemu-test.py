...
7. Sizes: the window at 640 x 480, 1024 x 768, 1920 x 1080 and 3200 x 1800 -
   the font chosen by the height (16, 20, 26 and 40 pixels), the buttons
   inside the screen, the colours on the screen, a click on a disk.
...
try:
    ...
    for xres, yres, font in ((640, 480, 16), (1024, 768, 20), (1920, 1080, 26), (3200, 1800, 40)):
        win = "%dx%d" % (xres, yres)
        S = "%dx%d: " % (xres, yres)
        ...
        try:
            check(S + "window", q.wait(r"gui: window %s font %d" % (win, font), 90), q.text()[-200:])
            q.wait(r"gui: shown")
